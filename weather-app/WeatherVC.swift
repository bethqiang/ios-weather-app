...
class WeatherVC: UIViewController, UITableViewDelegate, UITableViewDataSource, CLLocationManagerDelegate {
    ...
    func locationAuthStatus() {
        if CLLocationManager.authorizationStatus() == .authorizedWhenInUse { //if location already authorized
            currentLocation = locationManager.location
            Location.sharedInstance.latitude = currentLocation.coordinate.latitude //use singleton class
        ...
        } else { //if not already authorized, request authorization
            locationManager.requestWhenInUseAuthorization()
            locationAuthStatus()
        }
    }
